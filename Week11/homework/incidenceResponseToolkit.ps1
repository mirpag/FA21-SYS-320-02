...
function get_info() {
    ...
    #$sharePermissions = 
    Get-SmbShare | Export-Csv -Path "$mydir\sharePermissions.csv" -NoTypeInformation
    ...
    #$firewallSettings = 
    netsh advfirewall show currentprofile | Export-Csv -Path "$mydir\firewallSettings.csv" -NoTypeInformation
    ...
    | Export-Csv -Path "$mydir\cimInstance.csv" -NoTypeInformation
}
