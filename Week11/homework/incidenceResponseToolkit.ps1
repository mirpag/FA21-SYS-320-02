...
function get_info() {
    ...
    #$processes = 
    Get-Process | Select-Object ProcessName, Path | Export-Csv -Path "$mydir\processes.csv" -NoTypeInformation
    Write-Host "Get-Process information saved." 

    # 2. all registered services and the path to the executable controlling the service
    #$regServices = 
    Get-WmiObject -Class Win32_Service | Select-Object name, Path | Export-Csv -Path "$mydir\regServices.csv" -NoTypeInformation
    Write-Host "Registered services information saved."

    # 3. all tcp network sockets
    #$tcpNetworkSockets = 
    Get-NetTCPConnection | Export-Csv -Path "$mydir\tcpNetworkSockets.csv" -NoTypeInformation
    Write-Host "TCP Network Sockets saved." 

    # 4. all user account information (use wmi)
    #$userAcctInfo = 
    Get-WmiObject -Class Win32_UserAccount > $mydir\userAcctInfo.txt
    Write-Host "User account information saved."

    # 5. all network adapter configuration information
    #$networkAdaptherInfo = 
    Get-WmiObject -Class Win32_NetworkAdapterConfiguration > $mydir\networkAdapterInfo.txt
    ...
    #$sharePermissions = 
    Get-SmbShare > $mydir\sharePermissions.txt
    ...
    #$firewallSettings = 
    netsh advfirewall show currentprofile > $mydir\firewallSettings.txt
    ...
    | Export-Csv -Path "$mydir\cimInstance.csv" -NoTypeInformation -Force
}
...
function file_hash() {
    ...
    Get-FileHash -Path "$mydir\userAcctInfo.txt" -Algorithm MD5 | Add-Content hashes.txt
    Get-FileHash -Path "$mydir\networkAdapterInfo.txt" -Algorithm MD5 | Add-Content hashes.txt
    Get-FileHash -Path "$mydir\sharePermissions.txt" -Algorithm MD5 | Add-Content hashes.txt
    Get-FileHash -Path "$mydir\firewallSettings.txt" -Algorithm MD5 | Add-Content hashes.txt
